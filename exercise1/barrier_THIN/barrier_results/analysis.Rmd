...
# Now make the plot
Hockney.df <- tidyr::gather(barrier_linear.df %>% filter(Allocation=="core"), key = "Variable", value = "Value", -Processes, -Allocation, -Algorithm)

# Plot using ggplot
ggplot(Hockney.df, aes(x = Processes, y = Value, color = Variable)) +
  geom_point(data = subset(Hockney.df, Variable == "Latency"), size = 1) +
  geom_line(data = subset(Hockney.df, Variable == "HockneyLatency"), linetype = 1) +
  labs(title = "Linear Barrier: Hockney Model fit",
       x = "Processes",
       y = "Value") +
  scale_color_manual(values = c("Latency" = "grey19", "HockneyLatency" = "#00BFC1")) +
  theme_minimal()
```
