...
do
    ...
    command="mpirun -np $np --map-by ppr:1:core:PE=1 --bind-to core --mca coll_tuned_use_dynamic_rules true --mca coll_tuned_bcast_algorithm 0 ${src_path}osu_bcast -x 1000 -i 10000 -m 1:1 -f"
    # Execute the command, extract the numbers and append to the CSV file
    echo "Currentlty benchmarking Default with n. of processes= $np"
    latency_stats=$(eval $command | tail -n 1 | awk '{gsub(/ +/, ","); sub(/^[^,]+,/, ""); sub(/,[^,]+$/, ""); print}')
    echo "Default,$np,$latency_stats" >> $out_csv
done
