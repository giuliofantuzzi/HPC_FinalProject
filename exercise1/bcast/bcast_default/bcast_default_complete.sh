...
#out csv
out_csv="../bcast_results/bcast_default_complete.csv"
...
for map in $map_values; do
 for np in $np_values; do
    ...
    mpirun -np $np -map-by $map --mca coll_tuned_use_dynamic_rules true --mca coll_tuned_bcast_algorithm 0 osu_bcast -x 1000 -i 10000 | tail -n 21 \
    | awk -v np="$np" -v map="$map" '{printf "Default,%s,%s,%s,%s,%s\n",map,np,$1, $2, $3}' | sed 's/,$//' >> $out_csv
 done
done
