...
csv_file="../bcast_results/bcast_linear.csv"
...
for np in {2..256..1}
    ...
    command="mpirun -np $np --map-by core --mca coll_tuned_use_dynamic_rules true --mca coll_tuned_bcast_algorithm 1 ${exe_filepath}osu_bcast -x 1000 -i 10000 -m 1:1 -f"
    ...
    echo "linear,$np,$latency_stats" >> $csv_file
done
