...
It is basically what we did before, but now we considered a size of 160M data_t


First we upload the data from 1 to 64 processes:
```{r}
mpi_strong_1half.df<- read.csv("../timings/MPI_strong_160M_1-64.csv")
mpi_strong_1half.df<- mpi_strong_1half.df %>% 
                select(!Threads) %>%
                group_by(Processes) %>%
...
Then from 65 to 128:
```{r}
mpi_strong_2half.df<- read.csv("../timings/MPI_strong_160M_65-128.csv")
mpi_strong_2half.df<- mpi_strong_2half.df %>% 
                select(!Threads) %>%
                group_by(Processes) %>%
                summarise(Time = min(Time, na.rm = TRUE))
```
...
```{r}
mpi_strong.df<- rbind(mpi_strong_1half.df,mpi_strong_2half.df)
```
...
```{r}
mpi_weak_1half.df<- read.csv("../timings/MPI_weak_160M_1-64.csv")
mpi_weak_1half.df<- mpi_weak_1half.df %>% 
                select(!Threads) %>%
                group_by(Processes) %>%
                summarise(Time = mean(Time, na.rm = TRUE))
```
...
```{r}
mpi_weak_2half.df<- read.csv("../timings/MPI_weak_160M_65-128.csv")
mpi_weak_2half.df<- mpi_weak_2half.df %>% 
                select(!Threads) %>%
                group_by(Processes) %>%
                summarise(Time = mean(Time, na.rm = TRUE))
```
...
```{r}
mpi_weak.df<- rbind(mpi_weak_1half.df,mpi_weak_2half.df)
```
...
mpi_weak.df %>% 
  ...
  geom_hline(yintercept = mpi_weak.df$Time[1]*16,linetype="solid",color="navy",size=0.3)+
