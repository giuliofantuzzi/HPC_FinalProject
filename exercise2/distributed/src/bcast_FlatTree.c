...
#include "mpi.h"
//--------------------------------------------------------------------------------------
// Broadcast function using Flat Tree algorithm
void bcast_FlatTree(void* data,int count,MPI_Datatype datatype,int root,MPI_Comm communicator){
  int world_rank;
...
int main(int argc, char** argv){
  ...
  for (int i = 0; i < num_measurements; i++) {
    ...
    bcast_FlatTree(&data, 1, MPI_INT, 0, MPI_COMM_WORLD);
