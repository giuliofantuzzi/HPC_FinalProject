...
mpi_timings_1 %>%
  ...
  geom_point(size=2)+
    labs(title = "Time vs MPI Processes (Size=240M ; OMP_NUM_THREADS_NUM_THREADS=1)",
         x = "Processes",
         y = "Time") +
    theme_minimal()
#-------------------------------------------------------------------------------
# WITH 2 OMP_NUM_THREADS THREAD
...
mpi_timings_2 %>%
  ...
  geom_line(col="indianred1") +
  geom_point(size=2)+
  labs(title = "Time vs MPI Processes (Size=240M ; OMP_NUM_THREADS_NUM_THREADS=2)",
       x = "Processes",
       y = "Time") +
  theme_minimal()
#-------------------------------------------------------------------------------
# WITH 4 OMP_NUM_THREADS THREAD
...
mpi_timings_4 %>%
  ...
  geom_line(col="indianred1") +
  geom_point(size=2)+
  labs(title = "Time vs MPI Processes (Size=240M ; OMP_NUM_THREADS_NUM_THREADS=4)",
       x = "Processes",
       y = "Time") +
  theme_minimal()
#-------------------------------------------------------------------------------
# WITH 8 OMP_NUM_THREADS THREAD
mpi_timings_8<- read.csv("hybrid_scaling_240M_8threads.csv")
...
mpi_timings_8 %>%
  ...
  geom_line(col="indianred1") +
  geom_point(size=2)+
  labs(title = "Time vs MPI Processes (Size=240M ; OMP_NUM_THREADS_NUM_THREADS=8)",
       x = "Processes",
       y = "Time") +
  theme_minimal()
#-------------------------------------------------------------------------------
